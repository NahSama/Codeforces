...
function solve() {
	const size = nextInt();
	const arr = nextCharArray().join("");
	const specSize = nextInt();
	const spec = nextStrArray(specSize);
	const set = new Set(spec);
	let buffer = arr.split("");
	let lastPos = 0;
	...
	const maxSize = arr.length -1  - lastPos + 1;
	let count = 0;
	// console.log({maxSize, set, buffer, lastPos})
	while(true) {
		const tempBuffer = [];

		for (let i = 0; i < buffer.length; i++) {
			if (!set.has(buffer[i+1])){
